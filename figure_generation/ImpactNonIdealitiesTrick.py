...
data = pd.read_excel('TrickSimulationSummary.xlsx', 'Bonn')
data = pd.DataFrame(data)
...
plt.grid(axis='both')
plt.title(
    "Stuck Weight Offset Method", fontsize=titleSize, fontdict=dict(weight='bold'))
plt.xlabel("Stuck $R_{ON}$ and $R_{OFF}$ Devices (%)",
           fontsize=labelSize, fontdict=dict(weight='bold'))
plt.ylabel("Accuracy (%)", fontsize=labelSize, fontdict=dict(weight='bold'))
plt.locator_params(axis='y', nbins=6)
...
for j in range(len(dataset)):
    ...
    meanDict = {}
    stdDict = {}
    data = pd.read_excel('TrickSimulationSummary.xlsx', dataset[j])
    #print(dataset[j])
    data = pd.DataFrame(data)
    for seed in range(5, 9 + 1):
        indices = [z for z in range(
...
plt.savefig('SimulationSumaryTrick.pdf')
plt.savefig('SimulationSumaryTrick.svg')
plt.show()
