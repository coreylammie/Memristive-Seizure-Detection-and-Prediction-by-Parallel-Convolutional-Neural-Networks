...
seed = 42
patients = ['01'] # TO BE CHANGED to patients of the pretrained dataset and patients of the evaluation dataset
pretrained_dataset = 'CHBMIT'
evaluation_dataset = 'SWEC_ETHZ'
dataType = 'features'
numFold = 5
bS = 32
...
for patient in patients:
    ...
    for fold in range(0,numFold):
        ...
        network.load_state_dict(torch.load('{}_P{}F{}.pt'.format(pretrained_dataset,patients[0],fold)))
        ...
        for epoch in range(num_epoch_retrain):
            if epoch == 0:
                with torch.no_grad(): 
                    ...
                    df = df.append({'Patient': patient, 'Fold': fold, 'Epoch': -1, 'Loss': -1, 'Train Accuracy': -1, 'Test Accuracy': testAccuracy * 100.}, ignore_index=True)
            ...
            df.to_csv('Transfer_CHBMIT_SWEC_ETHZ.csv', index=False)